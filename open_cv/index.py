...
class Recognition:
     def ExtractNumber(self):
         ...
         #to find number plate measureing length between rectangles
          ...
          cv2.imwrite('./img/plate_th.jpg',th_plate)
          kernel = np.ones((3,3),np.uint8)
          er_plate = cv2.erode(th_plate,kernel,iterations = 1)
          er_invplate = er_plate
          cv2.imwrite('./img/er_plate.jpg',er_invplate)
          result = pytesseract.image_to_string(Image.open('./img/er_plate.jpg'), lang='kor')
